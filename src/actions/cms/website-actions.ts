...
export async function createWebsite(
  ...
  if (!isAdmin) {
    return {
      success: false,
      error: "Unauthorized: Only admins can create websites.",
    };
  }
...
export async function updateWebsite(
  ...
  if (!isAdmin) {
    return {
      success: false,
      error: "Unauthorized: Only admins can update websites.",
    };
  }
...
export async function deleteWebsite(id: string): Promise<ActionResponse<void>> {
  ...
  if (!isAdmin) {
    return {
      success: false,
      error: "Unauthorized: Only admins can delete websites.",
    };
  }

  try {
    const { error } = await supabase.from("cms_websites").delete().eq("id", id);
...
export async function getWebsitesByTenant(
  tenantId: string,
): Promise<ActionResponse<Website[]>> {
  const supabase = await createClient();
  ...
  try {
    const { data, error } = await supabase
      .from("cms_websites")
      .select(
        "id, name, domain, status, tenant_id, created_at, updated_at, description",
      )
    ...
    return { success: true, data: data as Website[] };
  } catch (error) {
...
export async function getAllWebsites(): Promise<ActionResponse<Website[]>> {
  ...
  if (!isAdmin) {
    return {
      success: false,
      error: "Unauthorized: Only admins can view all websites.",
    };
  }
