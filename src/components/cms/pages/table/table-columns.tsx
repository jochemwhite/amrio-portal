...
function PageTableActions({
  ...
  onStatusChange,
  websiteId,
}: PageTableActionsProps) {
  const { userSession } = useUserSession();
  const isSystemAdmin = userSession?.global_roles?.some(
        ...
        <AlertDialog>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Are you sure you want to delete "{page.name}"? This will
                permanently remove the page and all its schema data. This action
                cannot be undone.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
...
>[] => [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <Badge variant={variants[status] || variants.draft}>{status}</Badge>
      );
    },
